...
in some general circumstances, we use cos(theta) cuz that's more relevant (or if we don't want to use vectors)
_W= Fcos(theta) _ d*
...
- when _postive_ work is being done a body, the body goes faster as kinetic energy increases throughout the interval
- likewise when _negative_ work is being done, the body moves slower and kinetic energy decreases
- when the body _moves at a constant speed_, no work is done, and work = 0

work cannot occur without a force or displacement - work cannot be perpendicular either...
...
the moment of inertia is an objects tendency to keep doing what its been doing -> more inertia = more hard to change its momentum
~ in translational motion, the moment of inertia is dependent on an object's mass

## chapter 10 relationships

_torque is supa important for rotational motion_
_torque_ is a force perpendicular to the axis of rotation i think - it makes thing rotate faster or slower - net torque changes an object's angular velocity

- both the placement and the amount of force can affect the amount of torque
  - larger radius = more torque

- the angle between the applied force and the radius also affects the amount of torque
  - parallel would have like no torque

_T<sub>net</sub> = α x I (moment of inertia)*
an object with lots of inertia is harder to speed up/down
~ this moment of inertia is the*sum of all of the points of mass in an object_ the radius (distance of mass from axis of rotation) squared\_
ts depends on how the mass is distributed! -> \* the larger the radius, the larger the moment of inertia
you can also derive the moment of inertia equation by integrating the radius squared (r^2) over all mass of the object (dm)

anyways - torques, like forces, have the ability to do work!
the W<sub>torque</sub> = integral of torque over a certain angle
the more torque you have, the more work!

all of the KE that we have been talking about has been discussed _without_ rotational motion. but now that we have rotational motion in the picture, the KE from before must be differentiated by KE<sub>translational</sub>
on that note, _KE<sub>rotational</sub> = (1/2) x I x w^2_

- rotational motion has angular velocity, whereas translational motion has linear velocity
- the moment of inertia impacts rotational motion the same way that mass affects translational motion

we had uh linear momentum, which was p = mv but - you guessed it heh - we need angular momentum neow
this is known as _L = I x w_
and angular momentum is always conserved ! - you cannot create nor destroy angular momentum - it always has to go somewhere

## chapter 12 statics

FOR STATIC EQUILIIBRIUM THE SUM OF ALL FORCES IN THE SYSTEM MUST EQUAL ZERO
no acceleration at the center of mass
forces and torque forces cancel out

ok so _tension_
we've already talked about springs and whatnot - elasticity
but what if we have several springs?
we can uh calculate stuff using delta L = Nseries delta x ig
BUT to find the effective stiffness of a solid due to a VERTICAL chain of atoms in a Series :
keff = ks/Nseries
relate the system and observed stretch to the individual bonds
what if there's a thickness? like multiple springs compact in a material? how do parallel chains affect the overall stretch? side by side?
ok how find the effective stiffnss using all information ever

ok so um. streess= strain w/ a slope of youngs modulus
strain is proportional to stress UNTIL you hit the yield strength -> here the solid breaks

young's modulus -> elasticity
the higher it is -> the less elastic it is
delta L = (1/E)(F/A)(Inot)
(change in length) = (elasticity)(stress)(initial length)

so what is stress and what is strain?

stress is the force of an object divided by its cross sectional area (A)
strain is the change in length of an object divided by its original length

tensile stress stretches objects out and compressive stress compresses objects
sometimes its not js compressing and stretching - soometimes there's other forces like...
sheer stress - rectangle to parellelogram - parallel pushing???
higher in shear modulus -> harder to move

shrinking - applied force to every aspect of the object, like putting a sponge in water
more pressure -> more shrink - F/A as stress becomes pressure
changing volume?? bulk force??

## chapter 13 gravity

gravitational forces always point to each other :0 considering that gravity is proportional between the mass of two interacting bodies
it is also inversely proportional to the square of the distance between the two
G is so small that we don't normally notice it unless its hugemongous bodies
Newton's third law is the reason we know that the interacting bodies r proportional to one another : equal forces acting and cancelling

superposition principal: net sum of all individual effects - net gravitational force acting on the body is the vector sum of all forces (we know this)
real object - can be an integral
uniform sphere - mass can be treated as center where F is directed towards the center
if the radius of the shell is larger than the distance between an object and a shell's center, the force is zero (perfect symmetry)

- otherwise, we use Gmm/r^2 = F
  outside of the sphere: net gravity acts towards the center of the sphere
  inside of the sphere: only the mass inside the sphere with radius r results in gravity force

## chapter 14 ? fluids :PPPPPPPPPPPPPPPP

DENSITY IS SUPA IMPORTANT!!!!
anything that flows - liquid or gas - is a fluid (including air)
PRESSURE IS THE FORCE DISTRIBUTED OVER A SURFACE AREA (pressure is scalar btw)
zero pressure is in the absence of fluids - vacuum environment
...
P = rho _gravity_ height
_pascals principal_ - if you apply pressure to a confined fluid, the pressure in EVERY part of the fluid increases by that amount

how does pressure vary based off of the depth/height of a fluid?
well pressure comes from the weight within a fluid and can vary based off of temperature as well

there's gauge pressure thingies which can be negative or positive but lower than 1 atm or something - kinda like the minimum pressure in the system??? idk

### fluids in motion ~ fluid dynamics

buoyant force acting on an object partially/fully submerged in a liquid is equal to the weight of the volume of fluid displaced ...
= mass \* gravity
\_Fb = rho(density of liquid)gV

an object _sinks_ if it weighs more than the fluid it displaces => that is, if its average density is greater than the density of the fluid

an object _floats_ or rises to a surface if it weighs than the fluid being displaced - average density is less than fluid's density

_neutral buoyancy_ ~
an object hangs motionless if it weighs exactly the same

_partial submersion_ ~  
object floating is only partially submerged at equilibrium - the weight of the object is balanced by the buoyant force and will not be fully submerged

### continuity of fluid :0

viscosity - how hard it is to flow

_FLOW RATE Q_ - for an incompressible fluid, the volume flow rate must be conserved everywhere in the fluid
mass flow rate - always the same though a pipe -> delta m/delta t

area x velocity of flow stays constant

bernoulli's principal - the higher a fluid's velocity is through a pipe, the lower the pressure on the pipe's walls
bernoulli equation - P + 1/2(rho _v^2) + rho_ g \* y = a constant
energy is being conserved sooo make equation

torricellis theorem uses conservation of energy to find the velocity of fluid flowing from a small spout in a container
